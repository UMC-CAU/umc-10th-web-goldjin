...
import { useCartStore, useModalStore, type CartItemType } from "../zustand/store";
...
export const ShoppingCartPage = () => {
    ...
    <>
            ...
            isOpen && (
                ...
                <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-xl z-50 flex flex-col items-center">
                    ...
                    <div className="flex gap-4">
                        <button className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300" onClick={() => closeModal()}>아니요</button>
                        <button className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600" onClick={() => { clearCart(); closeModal(); }}>삭제</button>
                    </div>
                </div>
                </>
            )
        ...
        {cartItems.map((item: CartItemType, idx: number) => (
            <div key={item.id} className="flex items-center my-4 border-b border-gray-300 pb-8">
